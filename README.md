...
| `SLACK_BOT_IMAGE_ANALYSIS_MODEL`                     | Yes      | -        | Model name passed to the OpenCode Go chat model used to convert Slack image attachments into text before they reach the conversation agent's prompt or any `delegate_to_*` request (for example, `grok-4.5`). Shares `OPENCODE_API_KEY` and the same endpoint as `SLACK_BOT_CONVERSATION_AGENT_MODEL`.    |
