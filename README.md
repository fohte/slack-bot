...
| `SLACK_BOT_CONVERSATION_AGENT_PERSONA_PROMPT`        | No       | -                                              | System prompt applied to the conversation agent when set. Also used to paraphrase delegated agents' finalized replies into the same persona/tone before posting them to Slack.                                                                                                                              |
| `SLACK_BOT_LLM_API_KEY`                              | Yes      | -                                              | API key for the endpoint at `SLACK_BOT_LLM_BASE_URL`. Shared by the conversation agent and image analysis chat models.                                                                                                                                                                                      |
| `SLACK_BOT_LLM_BASE_URL`                             | No       | `https://opencode.ai/zen/go/v1`                | Base URL of the OpenAI-compatible endpoint used by both the conversation agent and image analysis chat models. Can be pointed at any OpenAI-compatible endpoint, not just OpenCode Go.                                                                                                                      |
| `SLACK_BOT_IMAGE_ANALYSIS_MODEL`                     | No       | (same as `SLACK_BOT_CONVERSATION_AGENT_MODEL`) | Model name passed to the OpenCode Go chat model used to convert Slack image attachments into text before they reach the conversation agent's prompt or any `delegate_to_*` request (for example, `grok-4.5`). Shares `SLACK_BOT_LLM_API_KEY` and the same endpoint as `SLACK_BOT_CONVERSATION_AGENT_MODEL`. |
...
| `A2A_NOTIFICATION_URL`                               | No       | - (none)                                       | Full URL of this service's own `POST /api/a2a/notifications` endpoint, as reachable by remote agents (may be an internal cluster address). Sent to remote agents as their push-notification callback target on every delegation/resume. Omitted means delegated tasks rely solely on `tasks/get` polling.   |
...
7. The `llm-agent` plugin also needs `files:read` (to resolve and download image attachments via `files.info`) and `channels:history` (to read prior thread messages via `conversations.replies` for context sync). Add `groups:history`, `im:history`, and/or `mpim:history` as well if the bot should be mentioned in private channels, DMs, or group DMs.

The `llm-agent` plugin shows an "is thinking…" loading indicator on accepted events, and reflects a delegated task's progress into it, using [`assistant.threads.setStatus`](https://api.slack.com/methods/assistant.threads.setStatus). Since Slack's [2026-03-05 scope update](https://docs.slack.dev/changelog/2026/03/05/set-status-scope-update/), this method works with the `chat:write` scope alone (already part of the minimum set from step 3) in both assistant threads and regular channels, so no separate **Agents & AI Apps** setup or `assistant:write` scope is required.
