...
| `OPENCODE_BASE_URL`                                  | No       | `https://opencode.ai/zen/go/v1`                | Base URL of the OpenAI-compatible endpoint used by both the conversation agent and image analysis chat models. Can be pointed at any OpenAI-compatible endpoint, not just OpenCode Go. Unprefixed for the same reason as `OPENCODE_API_KEY`.                                                              |
| `SLACK_BOT_IMAGE_ANALYSIS_MODEL`                     | No       | (same as `SLACK_BOT_CONVERSATION_AGENT_MODEL`) | Model name passed to the OpenCode Go chat model used to convert Slack image attachments into text before they reach the conversation agent's prompt or any `delegate_to_*` request (for example, `grok-4.5`). Shares `OPENCODE_API_KEY` and the same endpoint as `SLACK_BOT_CONVERSATION_AGENT_MODEL`.    |
