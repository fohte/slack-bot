...
| `SLACK_BOT_CONVERSATION_AGENT_MODEL`                 | No       | -                                                      | Model name passed to the OpenCode Go chat model used by the LangGraph-based conversation agent (for example, `opencode-go/gpt-5`).                                                                  |
| `SLACK_BOT_CONVERSATION_AGENT_PERSONA_PROMPT`        | No       | -                                                      | System prompt applied to the conversation agent when set.                                                                                                                                           |
| `OPENCODE_API_KEY`                                   | No       | -                                                      | API key for OpenCode Go's OpenAI-compatible endpoint. Unprefixed because multiple services share this credential.                                                                                   |
...
The conversation agent and remote agent registry variables above (`SLACK_BOT_CONVERSATION_AGENT_*`, `OPENCODE_API_KEY`, `REMOTE_AGENT_URLS`, `OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT`) are read by `loadConfig()` but not yet wired into `src/main.ts`; setting them currently has no runtime effect.
