...
| `SLACK_BOT_CONVERSATION_AGENT_MODEL`                 | No       | -                                                      | Model name passed to the OpenCode Go chat model used by the LangGraph-based conversation agent.                             |
...
| `REMOTE_AGENT_URLS`                                  | No       | - (none)                                               | Comma-separated list of A2A remote agent base URLs. Each becomes a delegation tool for the conversation agent.              |
| `OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT` | No       | `false`                                                | When `true`, includes full prompt/completion content in GenAI OpenTelemetry spans. Off by default since it may contain PII. |
