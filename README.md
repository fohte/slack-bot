...
| `A2A_NOTIFICATION_TOKEN`                             | Yes      | -        | Shared secret verified against the `X-A2A-Notification-Token` header on `POST /api/a2a/notifications`, the endpoint remote agents push completed or failed A2A tasks to.                            |
